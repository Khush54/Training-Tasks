...
function adults() {
    ...
    if (checkAdult.length != 0) {
        output += `There are ${(checkAdult.length)} adult passengers only and ${(passengers_details.length) - (checkAdult.length)} are minor passengers. `;
    }
    else{ output += `All passengers are adult`;}
    document.getElementById('result_box').innerHTML = output;
}

function minor() {
    let output = `<h4> Any Minor ? </h4> <br>`;
    let checkMinor = passengers_details.filter(i => i.age < 18)
    if (checkMinor.length == 0) {
        output += `No Passenger is minor.`;
    }
    output += `${checkMinor.length} are minor passengers. `;
    document.getElementById('result_box').innerHTML = output;
}
